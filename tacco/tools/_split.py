...
def split_observations(
    ...
    annotation_key
        ...
        generated by :func:`~tc.tl.annotate` with a `reconstruction_key`
        argument.
    result_key
    ...
    if mode == 'exact':
        ...
        sdata = split.split_beads(adata, annotation, profiles, min_counts=(0 if min_counts is None else min_counts), seed=seed, delta=delta, scaling_jobs=scaling_jobs, scaling_batch_size=scaling_batch_size, rounding_batch_size=rounding_batch_size, copy=False, map_all_genes=map_all_genes, verbose=verbose)

        layers = profiles.columns
        if mapping is not None:
            split.combine_layers(sdata, mapping, remove_old_layers=True)
            layers = layers.map(mapping).unique()

        mdata = split.merge_layers(sdata, layers, merge_annotation_name=merge_key, bead_annotation_name=obs_index_key, min_reads=min_counts)
...
def merge_observations(
    ...
    adata = split.merge_beads(adata, merge_annotation_name=annotation_key, bead_annotation_name=obs_index_key, min_counts=min_counts, mean_keys=mean_keys)
